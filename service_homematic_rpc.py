...
class HomematicRPCClient:
    ...
    def __init__(self, rpc_client: JsonRpcClient, username, password):
        ...
        print(f"Async HomematicRPCClient initialized for user '{username}'.")
    ...
    async def login(self):
        ...
        print("Async HomematicRPCClient: Attempting login...")
        if not self.username or not self.password:
            print("Async HomematicRPCClient Error: Username or Password not provided.")
        ...
        if response and "result" in response and response["result"] and response.get("id") == current_id:
            ...
            print(f"Async HomematicRPCClient: Login successful. Session ID: ...{self._session_id[-6:]}")
            self._login_attempts = 0
            self._next_req_id = 2
            await self._update_connection_status(response)
            return True
        else:
            print(f"Async HomematicRPCClient Error: Login failed. Response: {response}")
            self._session_id = None
            self._login_attempts += 1
            if self._login_attempts >= 3: 
                print("Async HC Error: Multiple failed login attempts.")
    ...
    async def logout(self):
        ...
        print("Async HomematicRPCClient: Logging out...")
        ...
        if logout_success: print("Async HomematicRPCClient: Logout successful.")
        else: print(f"Async HC Warning: Logout command failed. Response: {response}")
    ...
    async def _make_request(self, method, params=None, id_val=None, retries=1):
        ...
        if not current_session:
            print(f"Async HC: Not logged in for '{method}', attempting login.")
            if not await self.login():
                print(f"Async HC Error: Cannot make request '{method}', login failed.")
                return None
            current_session = self._session_id
        ...
        if response and "error" in response and response["error"]:
            ...
            if "session" in err_msg or \
               ...
               err_code == -1:
                ...
                print(f"Async HC: Detected potential session expiry/auth issue (Error: {err_data}). Re-logging in.")
            else:
        ...
        if session_expired:
            ...
            print("Async HC: Attempting re-login...")
            if await self.login():
                current_session = self._session_id
                print(f"Async HC: Re-login successful, retrying request '{method}'...")
                request_params["_session_id_"] = current_session
                ...
                print(f"Async HC Error: Re-login failed after session expiry detection.")
                return None
    ...
    async def get_device_ids(self):
        ...
        print("Async HC: Fetching device IDs (expects list of strings)...")
        ...
        if isinstance(result, list) and all(isinstance(item, str) for item in result): return result
             ...
             print(f"Async HC Warning: Device.listAll not list of strings! Got: {repr(result[:5])}")
             return [item for item in result if isinstance(item, str)]
        else:
             print(f"Async HC Warning: Device.listAll did not return list! Got: {type(result)}")
             return []
    ...
    async def get_device_details(self, device_id_str):
        ...
        if not isinstance(device_id_str, str):
             print(f"Async HC Error: get_device_details expects string ID, got {type(device_id_str)}")
             return None
        ...
        if result is not None and not isinstance(result, dict):
             print(f"Async HC Warning: Device.get for ID {device_id_str} not dict. Got: {type(result)}")
             return None
        return result
    ...
    async def list_all_rooms(self):
        ...
        print("Async HC: Fetching room IDs...")
        ...
        if isinstance(result, list) and all(isinstance(item, str) for item in result):
            ...
            print(f"Async HC Warning: Room.listAll did not return list of strings! Got: {type(result)}")
    ...
    async def get_room_details(self, room_id_str):
        ...
        if not isinstance(room_id_str, str):
            print(f"Async HC Error: get_room_details expects string ID, got {type(room_id_str)}")
            return None
        ...
        if result is not None and not isinstance(result, dict):
            print(f"Async HC Warning: Room.get for ID {room_id_str} not dict. Got: {type(result)}")
            return None
        return result
...
class HomematicDataService:
    ...
    def __init__(self, base_url, username, password, valve_device_type):
        ...
        print("HomematicDataService initialized.")
        if self._valve_device_list is not None:
            print(f"  Loaded {len(self._valve_device_list)} devices from cache.")
    ...
    def _load_cache(self):
        ...
        try:
            ...
            if isinstance(cached_data, list):
                ...
                print(f"Successfully loaded cache from {CACHE_FILENAME}")
            else:
                print(f"Warning: Cache file {CACHE_FILENAME} contained invalid data (not a list). Ignoring.")
                self._valve_device_list = None
        except OSError: # Catches FileNotFoundError and potentially other FS errors
            print(f"Cache file {CACHE_FILENAME} not found. Will perform discovery.")
            self._valve_device_list = None
        except ValueError: # Catches JSON decoding errors
            print(f"Warning: Cache file {CACHE_FILENAME} contained invalid JSON. Ignoring.")
            self._valve_device_list = None
    ...
    def _save_cache(self, device_list):
        ...
        try:
            ...
            print(f"Successfully saved {len(device_list)} devices to cache file {CACHE_FILENAME}")
        except OSError as e:
    ...
    async def _discover_valve_devices_and_rooms(self):
        ...
        """
        print("HomematicService: Discovering valve devices and rooms...")
        discovered_valves = []
        device_ids = await self._hm.get_device_ids()
        room_ids = await self._hm.list_all_rooms()

        if device_ids is None or room_ids is None:
            print("HomematicService: Failed to retrieve device or room list during discovery")
        ...
        for device_id in device_ids:
            ...
            if dev_addr and self.valve_device_type in dev_type:
                ...
                try:
                    ...
                    print(f"Warning: Could not convert device ID '{device_id}' to int for room search.")
        ...
        print(f"HomematicService: Discovery complete. Found {len(self._valve_device_list)} valve devices.")
    ...
    def update(self):
        ...
        if not self.paused():
            # print("WiFi connected, checking Homematic connection...")
        ...
        try:
            ...
            if not self._hm.is_logged_in():
                if not await self._hm.login():
                    # Login failed
                    print("HomematicService: Login failed")
            ...
            if valve_list_to_process is None:
                 ...
                 print("HomematicService Error: valve_list_to_process is unexpectedly None after discovery check.")
                 self.reporting_valves = -1
                 return False
            
            if not valve_list_to_process: # Discovery succeeded but found 0 devices
                print("HomematicService: No valve devices in list to process.")
                self.valve_devices = 0
                self.reporting_valves = 0
                self.avg_valve = 0.0
                self.max_valve = 0.0
            ...
            for valve_info in valve_list_to_process: # Iterate through list of dicts
                ...
                if pos_str is None:
                    ...
                    print(f"HomematicService Warning: Failed to get LEVEL for {iface}/{dev_addr}")
                    fetch_error_occurred = True
                    continue # Skip this device but continue with others
                try:
                    pos_val = float(pos_str)
                except ValueError:
                    print(f"HomematicService Warning: Invalid LEVEL value '{pos_str}' for {iface}/{dev_addr}")
            ...
            if fetch_error_occurred:
                print("HomematicService: Clearing cached valve list due to fetch error(s).")
                self._valve_device_list = None
            ...
            print(f"HomematicService: NetworkError during fetch: {ne}")
            ...
            print(f"HomematicService: General Exception during fetch_data: {e}")
    ...
    def force_rescan(self):
        ...
        print("HomematicService: Force rescan requested. Clearing device cache.")
        self._valve_device_list = None
